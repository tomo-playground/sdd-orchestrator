...
from orchestrator.tools.design import (
    _find_task_dir,
    _read_spec_status,
    _update_spec_status,
    auto_design_task,
...
class TestFindTaskDir:
    def test_found(self, task_env):
        ...
        with patch("orchestrator.tools.design.TASKS_CURRENT_DIR", current):
    ...
    def test_missing_dir(self, tmp_path):
        with patch("orchestrator.tools.design.TASKS_CURRENT_DIR", tmp_path / "nonexistent"):
...
"""
        with (
            patch("orchestrator.tools.design.query_agent", new_callable=AsyncMock) as mock_agent,
            patch(
                "orchestrator.tools.design._git_commit_and_push", new_callable=AsyncMock
            ) as mock_git,
        ):
...
"""
    ...
    async def test_timeout_keeps_pending(self, _patch_paths):
        ...
        with (
            patch("orchestrator.tools.design.query_agent", side_effect=slow_agent),
            patch("orchestrator.tools.design.DESIGN_TIMEOUT", 0.01),
        ):
            result = await auto_design_task("SP-099")
