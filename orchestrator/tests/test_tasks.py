...
from orchestrator.tools.tasks import do_approve_design, do_create_task, do_read_task
...
def task_dirs(tmp_path: Path):
    ...
    (task_dir / "spec.md").write_text(
        "# SP-086: Slack Templates\n\n> status: approved | approved_at: 2026-03-25\n\n## DoD\n\n1. Done\n",
        encoding="utf-8",
    )
...
class TestReadTask:
    ...
    async def test_read_done_task(self, task_dirs):
        current, done = task_dirs
        # Move task to done/
    ...
    async def test_read_legacy_md_file(self, task_dirs):
        current, done = task_dirs
...
class TestApproveDesign:
    async def test_approve_success(self, task_dirs):
        current, _ = task_dirs
        # Set status to design (approvable)
        spec = current / "SP-086_slack-templates" / "spec.md"
        spec.write_text(
            "# SP-086\n\n> status: design\n\n## DoD\n",
            encoding="utf-8",
        )
        with patch(
            "orchestrator.tools.tasks.git_commit_files",
    ...
    async def test_approve_pending_status(self, task_dirs):
        current, _ = task_dirs
        spec = current / "SP-086_slack-templates" / "spec.md"
        spec.write_text("# SP-086\n\n> status: pending\n", encoding="utf-8")
        with patch(
            "orchestrator.tools.tasks.git_commit_files",
            new_callable=AsyncMock,
            return_value=None,
    ...
    async def test_approve_git_failure(self, task_dirs):
        current, _ = task_dirs
        spec = current / "SP-086_slack-templates" / "spec.md"
        original_content = "# SP-086\n\n> status: design\n"
        spec.write_text(original_content, encoding="utf-8")
        with patch(
            "orchestrator.tools.tasks.git_commit_files",
            new_callable=AsyncMock,
        ...
        # spec.md must be rolled back to original (not left in approved state)
        assert spec.read_text(encoding="utf-8") == original_content


class TestCreateTask:
    async def test_create_basic(self, task_dirs, tmp_path):
        current, done = task_dirs
    ...
    async def test_create_collision_retries(self, task_dirs, tmp_path):
        ...
        with (
            patch("orchestrator.tools.tasks.next_sp_number", side_effect=mock_sp),
            patch(
                "orchestrator.tools.tasks.git_commit_files",
                new_callable=AsyncMock,
                return_value=None,
    ...
    async def test_create_git_failure_rollback(self, task_dirs, tmp_path):
        current, done = task_dirs
        backlog = tmp_path / "backlog.md"
        with patch(
            "orchestrator.tools.tasks.git_commit_files",
