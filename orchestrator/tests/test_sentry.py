...
from orchestrator.tools.sentry import (
...
class TestFetchSentryIssues:
    ...
    async def test_rate_limit_retry(self):
        ...
        with patch("orchestrator.tools.sentry.asyncio.sleep", new_callable=AsyncMock):
            result = await _fetch_sentry_issues(client, "test-project", since_hours=2)
...
class TestGetExistingSentryIds:
    ...
    async def test_parses_ids_from_gh(self):
        ...
        with patch(
            "orchestrator.tools.sentry.asyncio.create_subprocess_exec",
...
class TestSentryScan:
    ...
    async def test_no_token(self):
        with patch("orchestrator.tools.sentry.SENTRY_AUTH_TOKEN", ""):
    ...
    async def test_dedup_skips_existing(self):
        ...
        with (
            patch("orchestrator.tools.sentry.SENTRY_AUTH_TOKEN", "test-token"),
            patch(
                "orchestrator.tools.sentry._fetch_sentry_issues",
                new_callable=AsyncMock,
                return_value=[issue],
            ),
            patch(
                "orchestrator.tools.sentry._get_existing_sentry_ids",
                new_callable=AsyncMock,
                return_value={"111"},
            ),
            patch(
                "orchestrator.tools.sentry._create_github_issue",
                new_callable=AsyncMock,
            ) as mock_create,
        ):
            result = await do_sentry_scan()
    ...
    async def test_creates_and_triggers(self):
        ...
        with (
            ...
            patch(
                "orchestrator.tools.sentry._fetch_latest_stacktrace",
                new_callable=AsyncMock,
            ...
            patch(
                "orchestrator.tools.sentry._trigger_autofix",
                new_callable=AsyncMock,
            ) as mock_trigger,
        ):
            result = await do_sentry_scan()
