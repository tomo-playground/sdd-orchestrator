...
from orchestrator.tools.rollback import (
    _check_surge,
    _create_revert_pr,
    _handle_surge,
    _monitor_loop,
...
class TestCheckSurge:
    def test_surge_above_threshold(self):
        ...
        with patch("orchestrator.tools.rollback.ROLLBACK_ERROR_THRESHOLD", 5):
...
class TestMonitorLoop:
    ...
    async def test_no_surge_completes(self, store: StateStore):
        ...
        with (
            patch("orchestrator.tools.rollback._get_state_store", return_value=store),
            patch("orchestrator.tools.rollback.ROLLBACK_CHECK_INTERVAL", 1),
            patch("orchestrator.tools.rollback.ROLLBACK_MONITOR_DURATION", 1),
            patch(
                "orchestrator.tools.sentry.fetch_error_counts",
                new_callable=AsyncMock,
                side_effect=[baseline, current],
            ),
            patch(
                "orchestrator.tools.sentry.build_sentry_client",
                return_value=mock_client,
            ),
            patch(
                "orchestrator.tools.rollback._handle_surge", new_callable=AsyncMock
            ) as mock_surge,
            patch("orchestrator.tools.rollback.asyncio.sleep", new_callable=AsyncMock),
        ):
    ...
    async def test_surge_triggers_handle(self, store: StateStore):
        ...
        with (
            ...
            patch("orchestrator.tools.rollback.ROLLBACK_CHECK_INTERVAL", 1),
            patch("orchestrator.tools.rollback.ROLLBACK_MONITOR_DURATION", 1),
            patch("orchestrator.tools.rollback.ROLLBACK_ERROR_THRESHOLD", 5),
            patch(
                "orchestrator.tools.sentry.fetch_error_counts",
                new_callable=AsyncMock,
    ...
    async def test_fetch_failures_mark_monitor_failed(self, store: StateStore):
        ...
        with (
            patch("orchestrator.tools.rollback._get_state_store", return_value=store),
            patch("orchestrator.tools.rollback.ROLLBACK_CHECK_INTERVAL", 1),
            patch("orchestrator.tools.rollback.ROLLBACK_MONITOR_DURATION", 10),
            patch("orchestrator.tools.rollback.ROLLBACK_MAX_FETCH_FAILURES", 3),
            patch(
                "orchestrator.tools.sentry.fetch_error_counts",
                new_callable=AsyncMock,
...
class TestHandleSurge:
    ...
    async def test_revert_success(self, store: StateStore):
        ...
        with (
            patch("orchestrator.tools.rollback._get_state_store", return_value=store),
            patch(
                "orchestrator.tools.rollback._create_revert_pr",
                new_callable=AsyncMock,
                return_value=51,
            ),
            patch(
...
class TestCreateRevertPr:
    ...
    async def test_success(self):
        with (
            patch(
                "orchestrator.tools.rollback._get_pr_title",
                new_callable=AsyncMock,
                return_value="Add feature X",
            ),
            patch(
                "orchestrator.tools.rollback._ensure_label",
                new_callable=AsyncMock,
            ),
            patch(
                "orchestrator.tools.rollback._run_cmd",
                new_callable=AsyncMock,
                return_value=True,
            ),
            patch(
                "orchestrator.tools.rollback._run_cmd_output",
                new_callable=AsyncMock,
...
class TestStartPostMergeMonitor:
    ...
    async def test_creates_task(self):
        with patch(
            "orchestrator.tools.rollback._monitor_loop",
            new_callable=AsyncMock,
        ):
...
class TestMergePrAutoRollbackGuard:
    ...
    async def test_skips_monitor_when_sentry_token_empty(self):
        ...
        with (
            ...
            patch(
                "orchestrator.tools.github.asyncio.create_subprocess_exec", return_value=mock_proc
            ),
            patch("orchestrator.tools.github.asyncio.wait_for", return_value=(b"", b"")),
            patch("orchestrator.rules.can_auto_merge", return_value=(True, "")),
            patch("orchestrator.tools.rollback.start_post_merge_monitor") as mock_monitor,
            patch("orchestrator.config.SENTRY_AUTH_TOKEN", ""),
        ):
    ...
    async def test_starts_monitor_after_merge(self):
        ...
        with (
            ...
            patch("orchestrator.rules.can_auto_merge", return_value=(True, "")),
            patch("orchestrator.tools.rollback.start_post_merge_monitor") as mock_monitor,
            patch("orchestrator.config.SENTRY_AUTH_TOKEN", "fake-token"),
        ):
