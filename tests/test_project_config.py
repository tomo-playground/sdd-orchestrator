...
from sdd_orchestrator.project_config import (
    ProjectConfig,
    _build_config,
    _load_yaml_file,
    get_project_config,
...
class TestProjectConfigDefaults:
    """ProjectConfig defaults are empty (YAML-driven)."""

    def test_default_github_empty(self):
        cfg = ProjectConfig()
        assert cfg.gh_repo_owner == ""
        assert cfg.gh_repo_name == ""
        assert cfg.gh_issue_assignee == ""

    def test_default_sentry_empty(self):
        cfg = ProjectConfig()
        assert cfg.sentry_org == ""
        assert cfg.sentry_projects == ()
    ...
    def test_derived_properties(self):
        cfg = ProjectConfig(gh_repo_owner="acme", gh_repo_name="app")
        assert cfg.gh_repo_url == "https://github.com/acme/app"
        assert cfg.repo_full_name == "acme/app"
        assert cfg.repo_ssh_url == "git@github.com:acme/app.git"
...
class TestBuildConfig:
    ...
    def test_partial_yaml_uses_defaults(self):
        ...
        assert cfg.gh_repo_name == ""  # default empty
...
class TestLoadYamlFile:
    ...
    def test_file_found(self, tmp_path):
        ...
        with patch("sdd_orchestrator.project_config._get_project_root", return_value=tmp_path):
...
class TestLoadPrompt:
    ...
    def test_loads_and_substitutes(self, tmp_path):
        ...
        # Create sdd.config.yaml with test values
        yaml_content = {"project": {"github": {"owner": "acme", "repo": "app"}}}
        (tmp_path / "sdd.config.yaml").write_text(yaml.dump(yaml_content), encoding="utf-8")

        get_project_config.cache_clear()
        try:
            with (
                patch("sdd_orchestrator.project_config._PROMPTS_DIR", prompts_dir),
                patch("sdd_orchestrator.project_config._get_project_root", return_value=tmp_path),
            ):
    ...
    def test_missing_file_returns_empty(self, tmp_path):
        ...
        with patch("sdd_orchestrator.project_config._PROMPTS_DIR", prompts_dir):
...
class TestConfigCompatLayer:
    """config.py __getattr__ compatibility layer works with YAML config."""

    def test_gh_repo_owner_via_yaml(self, tmp_path):
        yaml_content = {"project": {"github": {"owner": "test-org", "repo": "test-repo"}}}
        (tmp_path / "sdd.config.yaml").write_text(yaml.dump(yaml_content), encoding="utf-8")

        get_project_config.cache_clear()
        try:
            with patch("sdd_orchestrator.project_config._get_project_root", return_value=tmp_path):
                from sdd_orchestrator import config

                assert config.GH_REPO_OWNER == "test-org"
        finally:
            get_project_config.cache_clear()
    ...
    def test_sentry_projects_returns_list(self, tmp_path):
        yaml_content = {"project": {"sentry": {"org": "org", "projects": ["backend", "frontend"]}}}
        (tmp_path / "sdd.config.yaml").write_text(yaml.dump(yaml_content), encoding="utf-8")
