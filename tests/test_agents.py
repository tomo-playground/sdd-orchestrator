...
from sdd_orchestrator.agents import build_cycle_prompt


class TestBuildCyclePrompt:
    def test_includes_scan_issues_when_enabled(self):
        with patch("sdd_orchestrator.agents.ENABLE_AUTO_ISSUES", True):
            prompt = build_cycle_prompt(1, None)
        assert "scan_issues" in prompt

    def test_excludes_scan_issues_when_disabled(self):
        with patch("sdd_orchestrator.agents.ENABLE_AUTO_ISSUES", False):
            prompt = build_cycle_prompt(1, None)
        assert "scan_issues" not in prompt
