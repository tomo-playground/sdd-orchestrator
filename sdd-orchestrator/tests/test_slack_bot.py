...
import sdd_orchestrator.tools.slack_bot as _slack_bot_module
from sdd_orchestrator.tools.slack_bot import (
    SlackBotListener,
    pause_orchestrator,
    resume_orchestrator,
    set_daemon,
...
class TestSlackBotNoTokens:
    ...
    async def test_no_tokens_skips_start(self):
        ...
        with (
            patch("sdd_orchestrator.config.SLACK_BOT_TOKEN", ""),
            patch("sdd_orchestrator.config.SLACK_APP_TOKEN", ""),
        ):
...
_PATCH_ALLOW_CHANNEL = "sdd_orchestrator.tools.slack_bot.SLACK_BOT_ALLOWED_CHANNEL"
_PATCH_ALLOW_USERS = "sdd_orchestrator.tools.slack_bot.SLACK_BOT_ALLOWED_USERS"
...
class TestSlackBotMention:
    ...
    async def test_agent_timeout_returns_error(self, bot):
        ...
        with (
            ...
            patch("sdd_orchestrator.tools.slack_bot.SLACK_BOT_AGENT_TIMEOUT", 0.01),
        ):
...
class TestPermissionCheck:
    ...
    async def test_denied_channel_silently_ignored(self, bot):
        ...
        with (
            patch("sdd_orchestrator.tools.slack_bot.SLACK_BOT_ALLOWED_CHANNEL", "C_ALLOWED"),
            patch.object(bot, "_ask_agent", new_callable=AsyncMock) as mock_agent,
    ...
    async def test_denied_user_gets_error_message(self, bot):
        ...
        with (
            patch("sdd_orchestrator.tools.slack_bot.SLACK_BOT_ALLOWED_CHANNEL", ""),
            patch("sdd_orchestrator.tools.slack_bot.SLACK_BOT_ALLOWED_USERS", "U_ADMIN"),
            patch.object(bot, "_ask_agent", new_callable=AsyncMock) as mock_agent,
            patch.object(bot, "_post_message", new_callable=AsyncMock) as mock_post,
        ):
            await bot._handle_mention(event, AsyncMock())
    ...
    def test_is_allowed_channel_empty_allows_all(self, bot):
        with patch("sdd_orchestrator.tools.slack_bot.SLACK_BOT_ALLOWED_CHANNEL", ""):
            assert bot._is_allowed_channel("ANY_CHANNEL") is True

    def test_is_allowed_user_empty_allows_all(self, bot):
        with patch("sdd_orchestrator.tools.slack_bot.SLACK_BOT_ALLOWED_USERS", ""):
...
_PATCH_CHANNEL = "sdd_orchestrator.tools.slack_bot.SLACK_BOT_ALLOWED_CHANNEL"
_PATCH_CHAT_INTERVAL = "sdd_orchestrator.tools.slack_bot.SLACK_BOT_CHAT_INTERVAL"
...
class TestBlockKit:
    def test_text_to_blocks_normal(self):
        ...
        from sdd_orchestrator.tools.slack_templates import agent_response_blocks
    ...
    def test_error_blocks_structure(self):
        ...
        from sdd_orchestrator.tools.slack_templates import error_blocks
