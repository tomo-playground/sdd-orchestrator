...
def _get_project_root() -> Path:
    return Path(os.environ.get("SDD_PROJECT_ROOT", Path.cwd()))
...
class ProjectConfig:
    ...
    def backlog_path(self) -> Path:
        return _get_project_root() / self.tasks_dir / self.backlog_file

    @property
    def tasks_current_dir(self) -> Path:
        return _get_project_root() / self.tasks_dir / "current"

    @property
    def tasks_done_dir(self) -> Path:
        return _get_project_root() / self.tasks_dir / "done"
...
def _load_yaml_file() -> dict:
    ...
    for candidate in _YAML_CANDIDATES:
        path = _get_project_root() / candidate
        if path.is_file():
            try:
...
def load_prompt(name: str) -> str:
    """Load a prompt from sdd_orchestrator/prompts/{name}.md.
