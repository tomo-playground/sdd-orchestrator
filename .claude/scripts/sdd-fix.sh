...
🤖 Auto-generated by sdd-fix" 2>/dev/null || true)

      if [ -n "$ORCH_PR_URL" ]; then
        echo "$(date '+%Y-%m-%d %H:%M') sdd-orchestrator PR 생성: $ORCH_PR_URL" >> "$LOG"
      fi
    else
      echo "$(date '+%Y-%m-%d %H:%M') sdd-orchestrator push 실패: $ORCH_BRANCH" >> "$LOG"
    fi
