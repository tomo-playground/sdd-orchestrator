...
변경 후 (`do_launch_sdd_run` 내부):
```
do_launch_sdd_run(task_id)
  1. worktree 생성
...
**핵심 구조**: Initializer/Evaluator = **SDK `query_agent()` 호출** (오케스트레이터 프로세스 내), Generator = **CLI subprocess** (기존 유지). 슬롯은 Generator만 소비, Initializer/Evaluator는 SDK 호출이므로 슬롯 미소비.
...
**실행 경계**: on-stop.sh(기계적 게이트)는 Generator CLI subprocess **내부** hook. Evaluator는 subprocess **종료 후** 오케스트레이터가 SDK로 호출.

```
CLI subprocess 내부:
  Generator 코딩 → on-stop.sh(pytest/vitest) → exit

CLI subprocess 외부 (_watch_process 후):
  complexity == "high" → Evaluator(SDK query_agent) → eval-report.json
    ├── PASS → state: running → done, PR 생성
    └── FAIL → Generator 재launch (최대 3회)
  complexity == "low" → Evaluator 스킵, 기존 흐름 유지
```

**피드백 루프의 재시도**: Evaluator FAIL → Generator **재launch**는 동일 run 내 retry로 취급. `get_consecutive_failures()` 카운터에 불포함 (연속 실패 blocked 판정과 분리).
...
sdd-orchestrator에 이미 Agent SDK(`ClaudeAgentOptions`)가 결합되어 있으며, `create_designer_options()` 및 `auto_design_task()` 패턴이 존재. Phase 1~2는 동일 패턴으로 확장.
