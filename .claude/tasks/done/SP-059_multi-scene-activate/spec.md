...
### 프롬프트 강화
- [x] Cinematographer 프롬프트(LangFuse 또는 prompt_builders_c.py)에서 scene_mode="multi" 출력 지시를 강화
- [x] 핵심 감정 장면에서 1-2씬은 multi로 지정하도록 명시적 규칙 추가
- [x] multi 씬 예시를 JSON output format에 포함

### Finalize 보정 완화
- [x] `_validate_scene_modes()`에서 multi → single 강제 전환 조건을 완화 (character_b_id 존재 시 multi 허용)
- [x] multi 씬 상한을 2개로 유지하되, 0개 강제 전환은 제거

### 동작 검증
- [x] dialogue 구조로 스토리보드 생성 시 scene_mode="multi" 씬이 1개 이상 생성됨
- [x] multi 씬의 이미지가 MultiCharacterComposer 경로로 생성됨 (BREAK 토큰 구조)
- [x] multi 씬에서 ControlNet/IP-Adapter가 정상적으로 비활성화됨 (기존 로직 유지)
