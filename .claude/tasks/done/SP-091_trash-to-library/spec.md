...
1. **SettingsShell** — TABS에서 Trash 항목 제거 (탭 3개 -> 2개)
2. **SubNavShell** — `footerLink` optional prop 추가 (탭바 우측 끝 ml-auto 배치)
3. **LibraryShell** — SubNavShell에 footerLink로 휴지통 링크 전달
4. **library/trash/page.tsx** — 신규 생성, 기존 TrashTab 컴포넌트 재사용
5. **settings/trash/page.tsx** — 삭제
6. **next.config.ts** — `/settings/trash` -> `/library/trash` permanent redirect 추가
