...
| `backend/services/agent/nodes/director_checkpoint.py` | 수정 | score_history 누적 로직 추가 |
...
#### 1-3. `director_node` 수정 (`director.py`)

> **Note**: checkpoint에서는 cineteam_call_count를 증분하지 않는다.
> checkpoint → proceed는 자연스러운 흐름이지 "Director의 반복 호출"이 아니며,
> DoD의 "Director→cinematographer 왕복 제한"은 director의 revise 결정만 카운트한다.
> revise_script 경로(writer → checkpoint → cinematographer)는 새 스크립트 평가이므로
> 글로벌 리비전 상한(`MAX_GLOBAL_REVISIONS`)이 별도로 가드한다.
...
|----------|------------------------|------|
| checkpoint → proceed → cinematographer (1회차) | 0 (미증분) | 정상 진행 — 초기 호출은 카운트 대상 아님 |
| director → revise_cinematographer (2회차) | 0 → 1 (director에서) | routing 통과 (1 < 3), cinematographer 진행 |
| director → revise_cinematographer (3회차) | 1 → 2 (director에서) | routing 통과 (2 < 3), cinematographer 진행 |
| director → revise_cinematographer (4회차) | 2 → 3 (director에서) | routing에서 3 ≥ MAX(3) → **강제 finalize** |

총 CineTeam 호출: 최대 3회 (초기 1 + revise 2). 4회째 차단. ✅ 스펙 충족.

### 엣지 케이스

1. **director가 revise_tts/revise_sound 결정**: cineteam_call_count 미증분. 해당 가드레일 미적용. ✅
2. **director가 revise_script 결정 후 다시 checkpoint → cinematographer**: cineteam_call_count 미증분 (checkpoint에서 증분하지 않음). 새 스크립트에 대한 정당한 CineTeam 호출이므로 카운트 대상 아님. 글로벌 리비전 상한(`MAX_GLOBAL_REVISIONS`)이 전체 비용을 별도 가드. ✅
...
5. **checkpoint → error → cinematographer**: cineteam_call_count 미증분 (checkpoint에서 증분하지 않음). error 경로도 초기 진입이므로 카운트 대상 아님. ✅
...
| `backend/services/agent/nodes/director_checkpoint.py` | 반환값 확장 (score_history 누적) |
...
## 설계 리뷰 결과 (난이도: 중 — Gemini 2라운드 + 에이전트 2라운드)

### Gemini 자문 (2라운드)
- R1: error 경로에서 cineteam_call_count 누락 위험 지적 → checkpoint 증분 제거로 해소
- R1: Reducer 패턴(`Annotated[int, operator.add]`) 제안 → 기존 코드베이스 `state.get()` 패턴이므로 불채택
- R1: revise_script 시 카운트 리셋 제안 → DoD "전체 비용 제한" 의도이므로 불채택
- R2: Oscillation(진동) 감지 제안 (max-min of 3) → MAX_CHECKPOINT_REVISIONS=3이므로 과도, 불채택
...
| 리뷰어 | 판정 | 주요 피드백 |
|--------|------|------------|
| Tech Lead | WARNING | cineteam_call_count를 checkpoint+director 양쪽에서 증분하면 실제 CineTeam 2회에서 차단됨 (DoD는 3회). 한 곳 통일 권장 |

**반영**: checkpoint 증분 제거. director의 `revise_cinematographer` 결정 시에만 증분. 동작 정의 테이블 + 엣지 케이스 보정 완료.

#### Round 2 (반영 후 재검증)
| 리뷰어 | 판정 | 주요 피드백 |
|--------|------|------------|
| Tech Lead | PASS | 동작 테이블 DoD 일치, revise_script 경로 안전, error 경로 안전, 테스트 정합성 확인 |