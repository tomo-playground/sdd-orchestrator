...
- [ ] `orchestrator/tools/sentry.py` — `sentry_scan` 도구: Sentry API로 미해결 에러 목록 조회
- [ ] 1시간마다 Sentry 스캔 → 신규 에러 발견 시 GitHub Issue 자동 생성 (기존 sentry-patrol 로직)
- [ ] Issue 생성 후 `gh workflow run sentry-autofix.yml` 자동 트리거
- [ ] autofix PR 생성 → SP-067의 PR 모니터링/자동 머지 파이프라인에 합류

### GitHub Actions 제어

- [ ] `orchestrator/tools/github.py` — `trigger_workflow` 도구: 워크플로우 수동 트리거
- [ ] `cancel_workflow` 도구: stuck 워크플로우 취소 (30분 초과 시)
- [ ] 오케스트레이터가 기존 GHA(claude-review, sdd-review, sdd-sync)의 상태를 모니터링하고, stuck/실패 시 재실행 또는 취소 판단

### Slack 알림

- [ ] `orchestrator/tools/notify.py` — `notify_human` 도구: Slack Webhook으로 메시지 전송
- [ ] 알림 조건:
...
- [ ] 일일 요약 리포트 (완료 태스크, 진행 중, 블로커)

### 일일 리포트

- [ ] 매일 09:00에 전일 활동 요약을 Slack으로 전송:
  - 완료된 PR/태스크
  - 진행 중인 작업
  - 블로커/실패 항목
  - Sentry 에러 현황
...
- [ ] 전체 파이프라인: spec 작성 → 자동 설계 → 자동 승인 → 자동 구현 → 자동 리뷰 → 자동 머지 → Sentry 모니터링
- [ ] 사람은 spec 작성 + BLOCKER 알림 대응만
- [ ] 린트 통과
