...
### confession 제거
- [ ] `config.py` STRUCTURE_METADATA에서 confession 항목 제거 (3종: monologue, dialogue, narrated_dialogue)
- [ ] `presets.py` confession 프리셋 제거 또는 monologue + emotional 매핑으로 전환
- [ ] `coerce_structure_id()`가 "confession" 입력 시 "monologue" 반환 (하위 호환)
- [ ] confession 참조 테스트 케이스를 monologue + emotional로 갱신

### tone 시스템 추가
- [ ] `config.py`에 `ToneMeta` 데이터클래스 + `TONE_METADATA` 정의 (intimate, emotional, dynamic, humorous, suspense)
- [ ] `storyboards` 테이블에 `tone: String(30), default="intimate"` 컬럼 추가 (Alembic 마이그레이션)
- [ ] Storyboard ORM 모델에 `tone` 필드 추가
- [ ] `/presets` API 응답에 `tones` 목록 포함
- [ ] Writer 노드에서 tone 값을 LangFuse 프롬프트 변수로 주입

### DB 마이그레이션
- [ ] `storyboards.structure = 'confession'` → `'monologue'` + `tone = 'emotional'` 데이터 마이그레이션
- [ ] 기존 non-confession 스토리보드는 `tone = 'intimate'` (기본값)
