...
3. `status: pending` → `status: design` 변경 (spec.md + state.db 동기화: `sqlite3 .sdd/state.db "INSERT INTO task_status (task_id, status, updated_at) VALUES ('SP-NNN', 'design', datetime('now')) ON CONFLICT(task_id) DO UPDATE SET status='design', updated_at=datetime('now');"` 실행)
...
2. `status: design` → `status: approved` + `approved_at: YYYY-MM-DD` 기록 (spec.md + state.db 동기화: `sqlite3 .sdd/state.db "INSERT INTO task_status (task_id, status, updated_at) VALUES ('SP-NNN', 'approved', datetime('now')) ON CONFLICT(task_id) DO UPDATE SET status='approved', updated_at=datetime('now');"` 실행)
