...
async def do_merge_pr(pr_number: int) -> dict:
    ...
    from sdd_orchestrator.rules import can_auto_merge
    ...
    try:
        from sdd_orchestrator.config import SENTRY_AUTH_TOKEN

        if SENTRY_AUTH_TOKEN:
            merge_sha = await _get_merge_sha(pr_number)
            if merge_sha:
                from sdd_orchestrator.tools.rollback import start_post_merge_monitor
...
async def do_trigger_workflow(workflow: str, *, inputs: dict[str, str] | None = None) -> dict:
    ...
    from sdd_orchestrator.config import GH_MONITORED_WORKFLOWS
