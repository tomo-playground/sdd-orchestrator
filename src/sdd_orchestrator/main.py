...
from sdd_orchestrator.agents import build_cycle_prompt, create_lead_agent_options
...
class OrchestratorDaemon:
    ...
    def _preflight_check(self) -> None:
        ...
        from sdd_orchestrator.config import SENTRY_AUTH_TOKEN, SLACK_BOT_TOKEN
    ...
    async def _maybe_start_slack_bot(self) -> None:
        ...
        from sdd_orchestrator.config import SLACK_APP_TOKEN, SLACK_BOT_TOKEN

        if not SLACK_BOT_TOKEN or not SLACK_APP_TOKEN:
            logger.info("Slack Bot tokens not set — bot listener disabled")
            return

        from sdd_orchestrator.tools.slack_bot import SlackBotListener, set_daemon

        set_daemon(self)
        self.slack_bot = SlackBotListener(mcp_server=self.mcp_server)

        # notify → SlackBot 경유로 모든 알림 발송
        from sdd_orchestrator.tools.notify import init_notify
    ...
    async def _auto_launch_approved(self) -> None:
        ...
        """
        from sdd_orchestrator.config import ENABLE_AUTO_RUN
        from sdd_orchestrator.tools.backlog import parse_backlog
        from sdd_orchestrator.tools.worktree import do_launch_sdd_run
    ...
    async def _maybe_send_daily_report(self) -> None:
        ...
        # _last_report_date check above ensures once-per-day semantics

        try:
            from sdd_orchestrator.tools.notify import send_daily_report
...
def cli_entry() -> None:
    """CLI entry point for `sdd-orchestrator` or `python -m sdd_orchestrator`."""
    _setup_logging()
