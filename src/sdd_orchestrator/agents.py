...
def build_cycle_prompt(cycle_number: int, previous_summary: str | None) -> str:
    ...
    next_num = 6
    if ENABLE_AUTO_ISSUES:
        steps.append(f"{next_num}. scan_issues → 미연결 GitHub Issue 스캔 + auto_create_task")
        next_num += 1
    steps.append(f"{next_num}. Decision Rules에 따라 액션 실행 (launch/merge/trigger/notify)")
    steps.append(f"{next_num + 1}. 종합 대시보드 출력")
