...
- **설계 깊이는 태스크 규모에 비례** (경량화 기준):
  - 변경 파일 ≤3 + 신규 함수 없음 → **설계 생략** (`pending` → 직접 `approved`. Hotfix 수준)
  - 변경 파일 4~7 + DB/API 변경 없음 → **간소화 설계** (구현방법 + 테스트 전략만)
  - 변경 파일 8+ 또는 DB/API 변경 → **풀 설계** (6항목)
- **자동 승인 조건** (오케스트레이터 전용 — SP-067~068):
  - 설계 생략 대상(변경 파일 ≤3 + 신규 함수 없음) → `pending` → 자동 `approved` → 자동 `/sdd-run`
  - 간소화 설계 대상(변경 파일 4~7 + DB/API 없음) → AI 설계 후 BLOCKER 없으면 자동 `approved`
  - 풀 설계 대상(변경 파일 8+ 또는 DB/API 변경) → **사람 승인 필수** (자동 승인 금지)
- **설계 커맨드**: `/sdd-design SP-NNN` — 코드 탐색 → 규모 판단 → 적정 깊이 설계 작성
...
| 자동 | `sdd-review.yml` (Claude Fix) | 리뷰 코멘트 → 자동 수정 |
