...
describe("buildNegativePrompt", () => {
  ...
  it("combines base and scene negative prompts", () => {
    const scene = { speaker: "speaker_1", negative_prompt: "bad hands" } as never;
  ...
  it("returns base only when scene negative is empty", () => {
    const scene = { speaker: "speaker_1", negative_prompt: "" } as never;
    const result = buildNegativePrompt(scene);
  ...
  it("deduplicates tokens", () => {
    const scene = { speaker: "speaker_1", negative_prompt: "lowres, extra" } as never;
    const result = buildNegativePrompt(scene);
