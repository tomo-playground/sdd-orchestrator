 ...
 */
export const normalizeOverlaySettings = (
  raw: Record<string, unknown> | null | undefined
): OverlaySettings => {
  const channelName = (raw?.channel_name ?? "") as string;
  const avatarKey = (raw?.avatar_key ?? slugifyAvatarKey(channelName)) as string;
  return {
    ...DEFAULT_OVERLAY_SETTINGS,
    ...(raw || {}),
 ...
 * Handles legacy field names and ensures all required fields are present.
 */
export const normalizePostCardSettings = (
  raw: Record<string, unknown> | null | undefined
): PostCardSettings => {
  const channelName = (raw?.channel_name ?? "") as string;
  const avatarKey = (raw?.avatar_key ?? slugifyAvatarKey(channelName)) as string;
