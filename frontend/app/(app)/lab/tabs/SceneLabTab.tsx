...
function matchRateBadge(rate: number) {
  if (rate > 0.8)
    return { label: "HIGH", bg: "bg-emerald-50", text: "text-emerald-700" };
  if (rate > 0.5)
    return { label: "MED", bg: "bg-amber-50", text: "text-amber-700" };
...
export default function SceneLabTab() {
  ...
  const loadHistory = useCallback(async () => {
    try {
      const res = await axios.get<{ items: HistoryItem[] }>(
        `${API_BASE}/lab/experiments`,
  ...
  const handleRun = useCallback(async () => {
    if (!sceneDescription.trim() || !characterId) return;
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="space-y-4 rounded-2xl border border-zinc-200 bg-white p-5">
          ...
          <button
            onClick={handleRun}
            disabled={loading || !sceneDescription.trim() || !characterId}
                  ...
                  >
                    ...
                    <td className="py-2 pr-4">
                      {item.match_rate != null ? (
                        <MatchBadge rate={item.match_rate} />
                      ) : (
                        "-"
                      )}
                    </td>
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-1">
        {tags.map((tag) => (
          <span
            key={tag}
            className={`rounded bg-zinc-50 px-1.5 py-0.5 text-[10px] ${color}`}
          >
            {tag}
          </span>
        ))}
    ...
    <div className="rounded-lg border border-zinc-100 bg-zinc-50 p-3">
      ...
      <div className="space-y-2">
        ...
        <div>
          ...
          <p className="mt-0.5 break-all text-xs text-zinc-700">
            {composedPrompt}
          </p>
        </div>
      </div>
