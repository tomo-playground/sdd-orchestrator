...
export default function CharactersSection({ showToast }: Props) {
  ...
  const openCreate = () => { setEditingCharacter(undefined); setShowModal(true); };
  const openEdit = (ch: Character) => { setEditingCharacter(ch); setShowModal(true); };
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-1 flex gap-1">
                    <button onClick={() => openEdit(ch)} className="text-[10px] text-zinc-500 hover:text-zinc-700 underline">
                      Edit
                    </button>
                    <button onClick={() => handleDelete(ch.id)} className="text-[10px] text-zinc-400 hover:text-red-500 underline">
                      Delete
                    </button>
                  </div>
                </div>
