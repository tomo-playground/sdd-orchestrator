...
# ── Phase B-3: Gaze 단조로움 보정 ──────────────────────────────────
# emotion → 권장 gaze 매핑 (looking_at_viewer 과다 사용 방지)
# 임계값 50% (expression의 60%보다 낮음): looking_at_viewer는 특정 1개 태그 점유율이므로 더 엄격하게 교정
...
# 한국어 emotion 별칭도 gaze에 매핑
for _ko, _en in _KOREAN_EMOTION_ALIASES.items():
    if _en in EMOTION_TO_GAZE and _ko not in EMOTION_TO_GAZE:
        EMOTION_TO_GAZE[_ko] = EMOTION_TO_GAZE[_en]


def diversify_gazes(scenes: list[dict]) -> None:
    """캐릭터 씬의 >50%가 동일 gaze이면 emotion 기반 교정.

    looking_at_viewer뿐 아니라 모든 dominant gaze에 대해 작동:
    1. char_scenes에서 gaze 분포 확인
    2. 최빈 gaze가 50% 초과 시:
       - emotion이 있는 씬에서 EMOTION_TO_GAZE로 대체
       - 연속 2씬 이상 같은 gaze 방지
    """
    from collections import Counter
    ...
    for s in char_scenes:
        ctx = s.get("context_tags") or {}
        gaze = _coerce_str(ctx.get("gaze"))
        gazes.append(gaze)

    counts = Counter(gazes)
    dominant_gaze, dominant_count = counts.most_common(1)[0]
    if not dominant_gaze or dominant_count <= len(char_scenes) * 0.5:
        return

    logger.info(
        "[Finalize] Gaze 단조로움 감지: '%s'가 %d/%d 씬 — emotion 기반 교정 시작",
        dominant_gaze,
        dominant_count,
        len(char_scenes),
    )
    ...
    for scene in char_scenes:
        ...
        current_gaze = _coerce_str(ctx.get("gaze"))
        if current_gaze != dominant_gaze:
        ...
        suggested = EMOTION_TO_GAZE.get(normalized_emotion) or EMOTION_TO_GAZE.get(emotion)
        if not suggested or suggested == dominant_gaze:
            prev_gaze = current_gaze
            continue

        # 연속 동일 gaze 방지
        if suggested == prev_gaze:
            alternatives = [
                "looking_down",
                "looking_up",
                "looking_afar",
            ...
            for alt in alternatives:
                if alt != prev_gaze and alt != dominant_gaze:
                    suggested = alt
                    break
            else:
                continue
    ...
    if corrected:
        logger.info("[Finalize] Gaze 보정 완료: %d씬 교체", corrected)


# ── Phase B-4: Action/Pose 반복 방지 ──────────────────────────────────
# emotion → 대안 action 매핑 (반복 탈출용)
...
def diversify_actions(scenes: list[dict]) -> None:
    ...
    for s in char_scenes:
        ctx = s.get("context_tags") or {}
        action = _coerce_str(ctx.get("action"))
        actions.append(action)

    counts = Counter(actions)
    dominant_action, dominant_count = counts.most_common(1)[0]
    if not dominant_action or dominant_count <= len(char_scenes) * 0.4:
        return

    logger.info(
        "[Finalize] Action 반복 감지: '%s'가 %d/%d 씬 — emotion 기반 교정 시작",
        dominant_action,
        dominant_count,
        len(char_scenes),
    )
    ...
    for scene in char_scenes:
        ...
        current = _coerce_str(ctx.get("action"))
        if current != dominant_action:
            prev_action = current
            continue
        ...
        suggested = _EMOTION_TO_ACTION.get(normalized)
        if not suggested or suggested == dominant_action:
            prev_action = current
            continue

        if suggested == prev_action:
            for alt in _EMOTION_TO_ACTION.values():
                if alt != prev_action and alt != dominant_action:
                    suggested = alt
                    break
            else:
                continue

        ctx["action"] = suggested
        prev_action = suggested
        corrected += 1

    if corrected:
        logger.info("[Finalize] Action 보정 완료: %d씬 교체", corrected)
