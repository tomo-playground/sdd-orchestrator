"""Prompt Composition Service with 12-Layer System.
...
class PromptBuilder:
    ...
    def _infer_layer_from_pattern(tag: str) -> int:
        ...
        tag_map = PromptBuilder._tag_to_layer_map()
        if tag in tag_map:
            return tag_map[tag]
    ...
    def _trigger_exists_in_layers(trigger: str, layers: list[list[str]]) -> bool:
        ...
        """
        norm = PromptBuilder._dedup_key(trigger)
        for layer_tokens in layers:
            for t in layer_tokens:
                if PromptBuilder._dedup_key(t) == norm:
                    return True
        return False
