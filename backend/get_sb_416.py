...
def get_storyboard_info(storyboard_id):
    with engine.connect() as conn:
        ...
        scenes_res = conn.execute(
            text('SELECT * FROM scenes WHERE storyboard_id = :id ORDER BY "order"'), {"id": storyboard_id}
        ).fetchall()
        ...
        for s in scenes_res:
            ...
            image_url = None
            if image_asset_id:
                asset_res = conn.execute(
                    text("SELECT * FROM media_assets WHERE id = :id"), {"id": image_asset_id}
                ).fetchone()
                if asset_res:
            ...
            score_res = conn.execute(
                text(
                    "SELECT match_rate FROM scene_quality_scores WHERE scene_id = :sid ORDER BY validated_at DESC LIMIT 1"
                ),
                {"sid": s_data["id"]},
            ).fetchone()
            if score_res:
