...
down_revision: Union[str, Sequence[str], None] = 'cdc167b32c1b'
...
def upgrade() -> None:
    ...
    op.add_column('creative_sessions', sa.Column('session_type', sa.String(length=20), server_default='free', nullable=False))
    op.add_column('creative_sessions', sa.Column('director_mode', sa.String(length=20), server_default='advisor', nullable=False))
    op.add_column('creative_sessions', sa.Column('concept_candidates', postgresql.JSONB(astext_type=sa.Text()), nullable=True))
    ...
    op.add_column('creative_traces', sa.Column('decision_context', postgresql.JSONB(astext_type=sa.Text()), nullable=True))
    op.add_column('creative_traces', sa.Column('retry_count', sa.Integer(), server_default='0', nullable=False))

    # Indexes for V2 queries
    op.create_index('ix_creative_traces_phase_step', 'creative_traces', ['session_id', 'phase', 'step_name'], unique=False)
