...
down_revision: str | Sequence[str] | None = "e0432dd57121"
...
def upgrade() -> None:
    ...
    noobai_id = conn.execute(
        sa.text("SELECT id FROM sd_models WHERE name = 'noobaiXLNAIXL_vPred10Version.safetensors'")
    ).scalar_one()

    smooth_neg_id = conn.execute(sa.text("SELECT id FROM embeddings WHERE name = 'SmoothNoob_Negative'")).scalar_one()

    smooth_quality_id = conn.execute(
        sa.text("SELECT id FROM embeddings WHERE name = 'SmoothNoob_Quality'")
    ).scalar_one()

    smooth_hands_id = conn.execute(
        sa.text("SELECT id FROM embeddings WHERE name = 'SmoothNegative_Hands'")
    ).scalar_one()
