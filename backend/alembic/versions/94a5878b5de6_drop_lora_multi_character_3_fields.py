...
down_revision: Union[str, Sequence[str], None] = 'c802651d6ede'
...
def downgrade() -> None:
    ...
    op.add_column('loras', sa.Column(
        'is_multi_character_capable', sa.BOOLEAN(),
        server_default=sa.text('false'), autoincrement=False, nullable=False,
    ))
    op.add_column('loras', sa.Column(
        'multi_char_weight_scale', sa.NUMERIC(precision=3, scale=2),
        autoincrement=False, nullable=True,
    ))
    op.add_column('loras', sa.Column(
        'multi_char_trigger_prompt', sa.VARCHAR(length=200),
        autoincrement=False, nullable=True,
    ))
