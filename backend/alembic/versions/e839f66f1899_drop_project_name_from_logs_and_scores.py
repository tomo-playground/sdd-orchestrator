...
down_revision: str | Sequence[str] | None = "42fc6b015f1a"
...
def downgrade() -> None:
    ...
    op.add_column(
        "scene_quality_scores", sa.Column("project_name", sa.VARCHAR(length=200), autoincrement=False, nullable=True)
    )
    ...
    op.add_column(
        "activity_logs", sa.Column("project_name", sa.VARCHAR(length=200), autoincrement=False, nullable=True)
    )
