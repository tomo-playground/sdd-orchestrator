...
async def test_characters_tags_passed_to_template(mock_validate, mock_call):
    ...
    with patch("services.agent.nodes.cinematographer.template_env") as mock_tenv:
        ...
        # characters_tags에 "A" 키가 있고 "brown_hair"가 포함되어야 함
        characters_tags = call_kwargs.kwargs.get("characters_tags")
        assert characters_tags is not None
        assert "A" in characters_tags
        assert "brown_hair" in characters_tags["A"]


@pytest.mark.asyncio
@patch("services.agent.tools.base.call_with_tools", new_callable=AsyncMock)
@patch("services.agent.nodes.cinematographer.validate_visuals")
async def test_characters_tags_none_when_no_character(mock_validate, mock_call):
...
@patch("services.agent.tools.base.call_with_tools", new_callable=AsyncMock)
@patch("services.agent.nodes.cinematographer.validate_visuals")
async def test_characters_tags_includes_lora(mock_validate, mock_call):
    ...
    def mock_execute(stmt):
        ...
        if call_count == 1:
            ...
            result.scalar_one_or_none.return_value = mock_lora
        return result
    ...
    with patch("services.agent.nodes.cinematographer.template_env") as mock_tenv:
        ...
        assert "purple_eyes" in tags_a
        assert "flat color" in tags_a
        assert "Midoriya_Izuku" in tags_a
