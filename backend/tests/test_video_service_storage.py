...
def mock_asset_service():
    with patch("services.video.upload.AssetService") as m:
        yield m
...
async def test_video_builder_uses_storage_and_asset_service(mock_asset_service, mock_storage, tmp_path):
    ...
    request = VideoRequest(scenes=[scene], storyboard_id=1, storyboard_title="test_video", project_id=1, group_id=1)

    # We need to mock some internal helper functions that VideoBuilder uses
    with (
        patch("services.video.VideoBuilder._setup_avatars"),
        ...
        mock_asset_service_instance.save_rendered_video.return_value = MagicMock(storage_key="test/video.mp4")
