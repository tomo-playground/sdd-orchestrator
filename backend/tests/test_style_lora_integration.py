"""Test cases for Style LoRA integration in Prompt Engine.
...
3. PromptService composition
4. PromptBuilder output
"""
...
class TestStyleLoRAIntegration:
    ...
    def test_prompt_builder_style_lora(self):
        """Test PromptBuilder with style_loras parameter."""
        builder = PromptBuilder(self.db)

        style_loras = [
            {"name": "harukaze-doremi-casual", "weight": 0.61, "trigger_words": ["hrkzdrm_cs"], "lora_type": "style"}
        ]
        ...
        prompt = builder.compose(tags=scene_tags, style_loras=style_loras)

        # Verify trigger word is included
        assert "hrkzdrm_cs" in prompt, f"Trigger word 'hrkzdrm_cs' not found in prompt: {prompt}"

        # Verify LoRA tag is included
        assert "<lora:harukaze-doremi-casual:0.61>" in prompt, f"LoRA tag not found in prompt: {prompt}"
        ...
        assert len(prompt) - lora_index < 100, "LoRA tag should be near the end of prompt (ATMOSPHERE layer)"

    def test_prompt_service_style_lora(self):
        """Test PromptService.generate_prompt_for_scene with style_loras."""
        service = PromptService(self.db)

        style_loras = [
            {"name": "harukaze-doremi-casual", "weight": 0.65, "trigger_words": ["hrkzdrm_cs"], "lora_type": "style"}
        ]

        scene_tags = ["standing", "smiling", "classroom"]

        # Test without character_id (generic mode)
        prompt = service.generate_prompt_for_scene(character_id=None, scene_tags=scene_tags, style_loras=style_loras)
    ...
    def test_scene_generate_request_schema(self):
        ...
        request = SceneGenerateRequest(
            ...
            character_id=1,
            style_loras=[{"name": "test-lora", "weight": 0.7, "trigger_words": ["test_trigger"], "lora_type": "style"}],
        )
    ...
    def test_multiple_style_loras(self):
        ...
        style_loras = [
            {"name": "chibi-laugh", "weight": 0.6, "trigger_words": ["chibi", "super_deformed"], "lora_type": "style"},
            {"name": "flat-color", "weight": 0.5, "trigger_words": ["flat color"], "lora_type": "style"},
        ]
    ...
    def test_style_lora_without_trigger_words(self):
        ...
        prompt = builder.compose(tags=["1girl"], style_loras=style_loras)
