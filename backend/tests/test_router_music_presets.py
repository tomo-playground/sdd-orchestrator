...
class TestMusicPresetsRouter:
    ...
    def test_create_preset(self, client):
        ...
        resp = client.post("/api/admin/music-presets", json=body)
    ...
    def test_get_preset(self, client):
        ...
        resp = client.get(f"/api/v1/music-presets/{pid}")
        assert resp.status_code == 200
        assert resp.json()["name"] == "Test BGM"

    def test_get_preset_not_found(self, client):
        resp = client.get("/api/v1/music-presets/9999")
    ...
    def test_update_preset(self, client):
        ...
        resp = client.put(f"/api/admin/music-presets/{pid}", json={"name": "After", "prompt": "new prompt"})
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_update_preset_not_found(self, client):
        resp = client.put("/api/admin/music-presets/9999", json={"name": "X"})
        assert resp.status_code == 404
    ...
    def test_delete_preset(self, client):
        ...
        resp = client.delete(f"/api/admin/music-presets/{pid}")
        assert resp.status_code == 200
    ...
    def test_delete_preset_not_found(self, client):
        resp = client.delete("/api/admin/music-presets/9999")
        assert resp.status_code == 404

    def test_list_after_create(self, client):
        client.post("/api/admin/music-presets", json={"name": "A"})
        client.post("/api/admin/music-presets", json={"name": "B"})
