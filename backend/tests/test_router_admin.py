...
class TestMigrateTagRules:
    """Test POST /admin/migrate-tag-rules endpoint."""

    def test_migrate_tag_rules_no_tags(self, client: TestClient, db_session):
        """Migration with no tags in DB reports errors."""
        response = client.post("/admin/migrate-tag-rules")
        assert response.status_code == 200
        data = response.json()

        assert data["success"] is True
        # All pairs should error because tags don't exist
    ...
    def test_migrate_tag_rules_with_tags(self, client: TestClient, db_session):
        ...
        assert data["success"] is True
        # At least one rule should be added (crying <-> laughing)
        assert data["total_added"] >= 1
        assert "crying <-> laughing" in data["added"]
    ...
    def test_migrate_tag_rules_idempotent(self, client: TestClient, db_session):
        ...
        # First migration
        response1 = client.post("/admin/migrate-tag-rules")
        data1 = response1.json()
        added_count = data1["total_added"]

        # Second migration
        response2 = client.post("/admin/migrate-tag-rules")
    ...
    def test_migrate_tag_rules_full_conflict_set(self, client: TestClient, db_session):
        ...
        assert data["success"] is True
        # Should have added multiple conflict rules
