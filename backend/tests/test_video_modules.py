...
class TestUploadResult:
    ...
    def test_fallback_uses_video_dir(self, _ac, mock_session_cls):
        from services.video.upload import upload_result

        # project/group/storyboard 모두 None → DB 조회 불가 → fallback
        builder = _make_builder(project_id_int=None, group_id_int=None)
        builder.request.storyboard_id = None
        result = upload_result(builder)
