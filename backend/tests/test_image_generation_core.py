...
class TestComposeSceneWithStyle:
    ...
    def test_calls_style_profile_then_v3(self):
        ...
        mock_db = MagicMock()
        # Character query returns mock (no negative_prompt → no merge)
        mock_char = MagicMock()
        mock_char.id = 10
    ...
    def test_character_negative_prompt_merged(self):
        """Character negative_prompt must be merged into negative."""
        mock_db = MagicMock()
        mock_char = MagicMock()
        mock_char.negative_prompt = "nsfw, revealing_clothes"
        mock_db.query.return_value.filter.return_value.first.return_value = mock_char
    ...
    def test_negative_prompt_merged(self):
        """Character negative_prompt (list[str]) must be merged into negative."""
        mock_db = MagicMock()
        mock_char = MagicMock()
        mock_char.negative_prompt = None
    ...
    def test_char_b_custom_negative_merged(self):
        """Multi-char scene: char_b's negative_prompt must be merged."""
        mock_db = MagicMock()

        mock_char_a = MagicMock()
        mock_char_a.id = 10
        mock_char_a.negative_prompt = "nsfw"

        mock_char_b = MagicMock()
        mock_char_b.id = 20
        mock_char_b.negative_prompt = "muscular, facial_hair"
