"""Seeds test data for Prompt Engine verification."""
...
def seed_test_data():
    ...
    try:
        ...
        if not lora:
            lora = LoRA(
                name="dummy_lora", display_name="Dummy LoRA", lora_type="character", trigger_words=["dummy_trigger"]
            )
            session.add(lora)
            session.commit()
        ...
        if not hana:
            hana = Character(
                name="Hana",
                description="Test character for testing",
                gender="female",
        ...
        for tag_name in identity_tags:
            ...
            if tag:
                link = (
                    session.query(CharacterTag)
                    .filter(CharacterTag.character_id == hana.id, CharacterTag.tag_id == tag.id)
                    .first()
                )
                if not link:
        ...
        if t1 and t2:
            existing_rule = (
                session.query(TagRule).filter(TagRule.source_tag_id == t1.id, TagRule.target_tag_id == t2.id).first()
            )
            if not existing_rule:
                rule = TagRule(
                    rule_type="conflict", source_tag_id=t1.id, target_tag_id=t2.id, message="Gender conflict detected"
                )
                session.add(rule)
        ...
        if t3 and t4:
            existing_rule = (
                session.query(TagRule).filter(TagRule.source_tag_id == t3.id, TagRule.target_tag_id == t4.id).first()
            )
            if not existing_rule:
                rule = TagRule(
                    rule_type="conflict", source_tag_id=t3.id, target_tag_id=t4.id, message="Quality conflict detected"
                )
                session.add(rule)
