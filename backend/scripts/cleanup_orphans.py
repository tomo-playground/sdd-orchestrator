...
def cleanup_orphans():
    ...
    try:
        # ActivityLog Cleanup
        log_query = db.query(ActivityLog).filter(ActivityLog.storyboard_id is None)
        ...
        if orphaned_logs_count > 0:
            db.query(ActivityLog).filter(ActivityLog.storyboard_id is None).delete(synchronize_session=False)
            print(f"Deleted {orphaned_logs_count} ActivityLogs")

        # SceneQualityScore Cleanup
        score_query = db.query(SceneQualityScore).filter(SceneQualityScore.storyboard_id is None)
        ...
        if orphaned_scores_count > 0:
            db.query(SceneQualityScore).filter(SceneQualityScore.storyboard_id is None).delete(synchronize_session=False)
