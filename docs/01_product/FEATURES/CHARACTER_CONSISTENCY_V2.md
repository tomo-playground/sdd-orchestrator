...
| Multi-Character (일반 프롬프트) | ✅ 완료 | V-Pred txt2img 2인 생성 | 10/10 성공, BLOCKER 방어 미완 |
...
**파일**: `pipeline/cinematographer` LangFuse 프롬프트, `storyboard/default` LangFuse 프롬프트, `services/agent/nodes/finalize.py`
...
### ~~Sub-Phase C: Dual IP-Adapter 활성화~~ (보류)

> **보류 사유** (03-14 검증): V-Pred IP-Adapter 호환성은 확인됨(ip-adapter-plus-face_sdxl, NOOB-IPA-MARK1 양쪽 동작).
> 그러나 V-Pred 모델 자체가 프롬프트 준수율이 높아 IP-Adapter 유무 차이가 **체감 미미**.
> Dual Unit 추가 효과 불확실 + VRAM 리스크. LoRA 트레이닝(30-D)이 근본 해결책으로 더 적합.
...
| Gemini 템플릿 2인 지시 | ✅ 완성 | `storyboard/dialogue` LangFuse 프롬프트 — is_multi_character_capable 조건부 |
...
| `storyboard/dialogue` LangFuse 프롬프트 | `is_multi_character_capable` 조건 유지 (변수 소스만 변경) |
| `storyboard/narrated-dialogue` LangFuse 프롬프트 | 동일 |
...
| ID | 실험 | 방법 | 판단 기준 |
|----|------|------|----------|
| **EXP-1** | V-Pred BREAK 토큰 호환성 | ✅ 완료 | 분리 효과 미미, 인원 수 제어에 약간 도움. **유지** |
| **EXP-2** | CFG 4.5 vs 5.5 multi 씬 | ✅ 완료 | 유의미 차이 없음. **CFG 4.5 유지** |
| **EXP-3** | wide_shot vs upper_body 2인 배치 | ✅ 완료 | wide_shot 80% 최고. **wide_shot 유지** |
| **EXP-4** | ADetailer 2인 얼굴 감지 | ✅ 완료 | 5/5 성공, 오감지 0건. **ADetailer 유지** |
