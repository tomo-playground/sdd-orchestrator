...
| 2 | `pipeline/director-plan` LangFuse 프롬프트 인벤토리 섹션 추가 | 캐릭터 목록 + 구조 메타 + 스타일 목록 주입 + CoT 캐스팅 가이드 |
...
기존 `pipeline/director-plan` LangFuse 프롬프트에 인벤토리 + CoT 캐스팅 가이드 추가:
...
| `pipeline/director-plan` LangFuse 프롬프트 | 완료 | Project Brief + Mission + JSON Format |
...
| `pipeline/director-plan` LangFuse 프롬프트 렌더링 (인벤토리 주입 + CoT 가이드) | ~5 |
...
| `pipeline/director-plan-lite` LangFuse 프롬프트 | C | 경량 캐스팅 프롬프트 |
...
| `pipeline/director-plan` LangFuse 프롬프트 | A | 인벤토리 섹션 + CoT 캐스팅 가이드 + casting JSON format |
