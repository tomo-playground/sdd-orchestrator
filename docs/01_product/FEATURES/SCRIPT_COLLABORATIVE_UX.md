...
### 생성 후 대화형 수정 루프
- [x] CompletionCard 이후 ChatInput 계속 활성화 (isEditMode 전환)
- [x] `POST /scripts/edit-scenes` — Gemini 단일 호출로 변경된 씬만 반환
- [x] `SceneEditDiffCard` — Before/After 단어 단위 diff (LCS 알고리즘)
- [x] Accept/Reject 버튼 → 씬 업데이트 + 자동 저장
- [x] 연속 편집 지원 (적용 후 다시 수정 지시)
- [x] `useSceneEditActions` 훅으로 편집 로직 분리
