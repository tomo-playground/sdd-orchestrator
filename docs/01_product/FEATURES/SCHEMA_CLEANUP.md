...
| `projects` | `default_character_id` | `character_id` | 프로젝트의 선택된 캐릭터 (완료) |
| `projects` | `default_style_profile_id` | `style_profile_id` | 프로젝트의 선택된 스타일 (완료) |
...
ALTER TABLE projects RENAME COLUMN default_character_id TO character_id;  -- 완료
ALTER TABLE projects RENAME COLUMN default_style_profile_id TO style_profile_id;  -- 완료
```
...
| 테이블 | 컬럼 | 문제 | 해결 |
|--------|------|------|------|
| `storyboards` | `character_id` (旧 `default_character_id`) | FK 없음 | Phase 1.7에서 컬럼 제거 예정 |
| `storyboards` | `style_profile_id` (旧 `default_style_profile_id`) | FK 없음 | Phase 1.7에서 컬럼 제거 예정 |
