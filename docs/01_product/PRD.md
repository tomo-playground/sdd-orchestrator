...
V3 아키텍처 전환 (Storyboard-Centric + DB-Driven) 완료 및 Project/Group 시스템 도입을 반영합니다.

## 변경 이력

| 버전 | 날짜 | 주요 변경사항 |
|------|------|--------------|
| v3.1 | 2026-02-02 | Project/Group 시스템, Cascading Config, Render/Voice Presets, Qwen TTS 통합 |
...
*   **Architecture**:
    *   **Project/Group System**: Project (채널) → Group (시리즈) → Storyboard 계층 구조.
    *   **Cascading Config**: Project → Group → Storyboard 설정 상속 엔진 (Render Preset, Character, Style Profile).
    ...
    *   **Activity Logs**: 생성 이력 통합 (`activity_logs` 테이블).
...
*   **Production**:
    *   **Render Presets**: 재사용 가능한 렌더링 설정 번들 (BGM, 폰트, 레이아웃, Ken Burns, TTS 등 14개 필드).
    ...
    *   **Audio**: **Qwen TTS** (로컬 VoiceDesign) + Edge-TTS (Fallback) + **Random BGM** + Audio Ducking.
    *   **Voice Presets**: 음성 프리셋 관리 (생성/업로드, 프리뷰 미리듣기, Render Preset 연동).
*   **Quality Control**:
...
품질 안정화 및 고도화를 위해 예정된 작업입니다 (`docs/01_product/ROADMAP.md` 참조).
*   **Channel DNA** (Phase 7-2 Phase 2): 톤/세계관 자동 주입, Tag Intelligence.
*   **Multi-Character UI**: 2인 이상 캐릭터의 대화 및 상호작용 (DB 스키마 완료, UI 대기).
*   **Scene Builder UI**: 장면별 배경/시간/날씨 컨텍스트 태그 선택 UI.
*   **Character Builder Wizard**: 단계별 캐릭터 생성 가이드.
...
*   **Process**:
    ...
    3. Backend: FFmpeg로 [Motion Video + Scene Text + Transition + Audio] 합성.
       - **Layout Handling**:
...
*   **Database**: PostgreSQL (로컬/Docker)
    *   V3 Schema: projects, groups, storyboards, scenes, character_tags, scene_tags, scene_character_actions, activity_logs, render_presets, voice_presets, tag_rules, tag_aliases, tag_filters
    *   마이그레이션: Alembic (V3 Baseline + Media Assets + Render/Voice Presets)
