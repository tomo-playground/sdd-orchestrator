...
| **Phase 30 (Character Consistency V2)** | **진행 중** — A~N(11개) + **O(Multi-Character Scene) 완료** (03-14). LoRA multi 3필드 제거+게이트 단순화(Dialogue+2캐릭터), BREAK 토큰 캐릭터 분리+per-BREAK dedup, Finalize BLOCKER 방어(ControlNet/IP-Adapter 비활성화+Narrator 보정+구조 제한), Generation 경로 방어(soft-deleted/동일ID/negative 주입), TTS speaker 캐시 키. G(Inpaint)→O로 대체 완료. C(Dual IP-Adapter) D(LoRA 트레이닝) SDXL 전환 후 재검토 |
