...
| # | 작업 | 난이도 | 상태 |
|---|------|--------|------|
| 4 | **씬 편집 탭 분리**: SceneCard 내부를 "대본/비주얼/고급" 3탭으로 분리. 기존 컴포넌트 재배치 수준, 신규 UI 없음 | 1-2시간 | [ ] |
| 5 | **노트 & 메모**: scenes.notes 컬럼 추가 (Text, nullable) + Alembic 마이그레이션. 대본 탭 하단에 자유 메모 입력란 | 1시간 | [ ] |
| 6 | **씬별 AI 재생성**: SceneCard "AI 재생성" 버튼 + 모달. 수정 지시 텍스트 입력 + 재생성 범위 선택 (대본/비주얼/캐릭터액션). 전체 스토리보드 컨텍스트 유지하면서 특정 씬만 Gemini 재호출 | 반나절 | [ ] |

**DoD (Phase B)**:
- [ ] 탭 분리: 대본(스크립트+글자수+메모) / 비주얼(이미지+프롬프트+match_rate) / 고급(캐릭터액션+SD파라미터) 3탭 전환
- [ ] 노트: DB 마이그레이션 + API PATCH 지원 + 프론트엔드 textarea 저장 (autoSave 연동)
- [ ] AI 재생성: `/storyboard/{id}/scenes/{scene_id}/regenerate` API + 범위 파라미터 (script/visual/actions) + Gemini 부분 재호출 + 기존 씬 데이터 병합
- [ ] AI 재생성 모달: 수정 지시 텍스트 입력 + 체크박스 범위 선택 + 미리보기(diff)
- [ ] DBA 리뷰 통과 (scenes.notes 스키마 변경)
- [ ] 빌드 PASS + 신규 테스트 추가
     ...
     P0/P1 Fixes          Refactoring          CI + Soft Delete    TTS/Voice/BGM     Pose Control      New Features
               ...
               7-2 (Project/Group) → 7-3 (Production Workspace) → 7-4 (Studio + Script Vertical) → 7-5 (UX Quality)
                Cascading Config      재료 독립 페이지              Studio 코디네이터 + 대본 버티컬    피드백/안전성/일관성
                                                                                                            ↓
                                                                              7-6 (Scene UX Enhancement) → 8 (Multi-Style)
                                                                               Figma 기반 씬 편집 개선        Future
```
...
- **Phase 7-6**: Scene UX Enhancement **3/6** (Phase A 3건 완료 2026-02-13, Phase B 3건 대기)
...
| 2 | 7-6 B #4~5 | 탭 분리 + 노트 기능 | ~2시간, 편집 효율 향상 |
| 3 | 7-6 B #6 | 씬별 AI 재생성 | 반나절, 핵심 생산성 기능 |
