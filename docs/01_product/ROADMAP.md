...
### Phase B: 피드백 품질 + 에러 복구 + 성능

| # | 작업 | 분류 | 발견자 | 상태 |
|---|------|------|--------|------|
| 10 | 에러 메시지 구조화: `str(exc)` → 사용자 친화적 한국어 메시지 래핑 (20+ 라우터) | 보안/UX | BE, Security | [ ] |
| 11 | 이미지 생성 SSE 진행률 도입 (`/scene/generate` SD progress API 활용) | UX | BE, FFmpeg, Prompt | [ ] |
| 12 | ScenesTab `useStoryboardStore()` → 개별 selector 분리 (리렌더링 최적화) | 성능 | FE | [ ] |
| 13 | 빈 catch 블록 17건 정리 (에러 로깅/Toast 추가) | 안정성 | FE, QA | [ ] |
| 14 | 파일 업로드 MIME 화이트리스트 + 파일 크기 제한 (backgrounds) | 보안 | Security | [ ] |
| 15 | MinIO 기본 credential 제거 + startup 필수 검증 | 보안 | Security | [ ] |
| 16 | Pydantic 주요 필드 `max_length` 추가 (Gemini 전달 필드 우선) | 보안 | Security | [ ] |
| 17 | 렌더링 ETA 표시 (elapsed 기반 예상 잔여 시간) | UX | FFmpeg | [ ] |
| 18 | Duration 입력 JS 검증 (NaN/범위 클램핑) | 안정성 | QA | [ ] |
| 19 | 스토리보드 재생성 시 기존 씬 덮어쓰기 경고 | 안정성 | QA | [ ] |
| 20 | SSE 연결 끊김 자동 재연결 (최대 3회) | 안정성 | FFmpeg | [ ] |

### Phase C: 구조적 개선 (데이터 안전성 + 컴포넌트 구조)

| # | 작업 | 분류 | 발견자 | 상태 |
|---|------|------|--------|------|
| 21 | 씬 Client-Side UUID 도입 (ID 재할당 레이스 컨디션 근본 해결) | 안정성 | QA, FE | [ ] |
| 22 | Optimistic Locking (`version` 컬럼 + 409 Conflict, 멀티탭 데이터 유실 방지) | 안정성 | DBA | [ ] |
| 23 | 핵심 엔드포인트 `response_model` 정리 (storyboard/scene/video 우선) | API 품질 | BE | [ ] |
| 24 | 페이지네이션 통일 (스토리보드/태그/캐릭터 목록) | 성능 | BE, DBA | [ ] |
| 25 | Skeleton 로딩 컴포넌트 도입 (LoadingSpinner → SkeletonCard) | UX | UI/UX | [ ] |
| 26 | Soft Delete 복원 정합성 (삭제 timestamp 기반 필터 / batch_id 추가) | 안정성 | DBA | [ ] |
| 27 | Active 상태 스타일 토큰 통일 (TAB_ACTIVE / FILTER_PILL_ACTIVE) | 일관성 | UI/UX | [ ] |
| 28 | `list_storyboards` joinedload → selectinload 전환 (카르테시안 곱 방지) | 성능 | DBA | [ ] |
| 29 | Backgrounds N+1 해결 (`joinedload(Background.image_asset)`) | 성능 | BE | [ ] |
| 30 | LocalStorage Path Traversal 방어 (`resolve()` + base_dir 검증) | 보안 | Security | [ ] |
...
Phase 6-5 (Stability) → 6-6 (Code Health) → 6-7 (Infra/DX) → 6-8 (Local AI) → 7-0 (ControlNet) → 7-1 (UX/Feature)
     P0/P1 Fixes          Refactoring          CI + Soft Delete    TTS/Voice/BGM     Pose Control      New Features
                                                                                                       + Creative Lab
                                                                                                            ↓
               7-2 (Project/Group) → 7-3 (Production Workspace) → 7-4 (Studio + Script Vertical) → 7-5 (UX Quality) → 8 (Multi-Style)
                Cascading Config      재료 독립 페이지              Studio 코디네이터 + 대본 버티컬    피드백/안전성/일관성     Future
```
...
**Tier 2 — UX Quality & Reliability**
| 순위 | 출처 | 작업 | 근거 |
|------|------|------|------|
| ~~5~~ | ~~7-5 A~~ | ~~Quick Wins 9건 (Toast큐, confirm교체, dirty가드 등)~~ | ~~완료 (2026-02-12)~~ |
| 6 | 7-5 B | 피드백 품질 11건 (에러메시지, SSE진행률, 보안검증 등) | 사용자 경험 핵심 |
| 7 | 7-5 C | 구조적 개선 10건 (UUID, Optimistic Lock, Skeleton 등) | 데이터 안전성 |

**Tier 3 — Feature 확장**
| 순위 | 출처 | 작업 | 근거 |
|------|------|------|------|
| 8 | 7-1 #8 | Character Builder 위저드 | 캐릭터 AI 에이전트화 선행 |
| 9 | 7-2 P2 | Channel DNA + Tag Intelligence | 프로젝트 차별화 |
