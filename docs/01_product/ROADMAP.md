...
| **Phase 30 (Character Consistency V2)** | **진행 중** — A~M(11개 Sub-Phase) 완료. **O(Multi-Character Scene) 계획 완료/구현 미착수** — V-Pred 2인 프롬프트 테스트 10/10 성공, Regional Prompter/Forge Couple/MultiDiffusion V-Pred 미호환 확인, 일반 프롬프트 방식 채택. Composer 수정(2people/couple 시그널)+Finalize scene_mode 자동설정+BLOCKER 대응(ControlNet/IP-Adapter/ADetailer multi 비활성화)+Gemini 템플릿 2인 지원. C(Dual IP-Adapter) D(LoRA 트레이닝) G(멀티캐릭터 Inpaint→**O로 대체 검토**) 미착수 |
