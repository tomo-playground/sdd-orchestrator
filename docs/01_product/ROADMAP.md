...
- **Phase 29-B TTS 프리뷰→렌더 연결 + 씬 필드 소실 수정** (03-05): ① tts_asset_id 기반 TTS 재사용 — 프리뷰 TTS를 렌더 시 재생성하지 않고 직접 사용(Scene DB 컬럼 추가, Frontend/Backend 파이프라인 연결). ② 씬 필드 소실 근본 원인 수정 — `mapDbScenes()`에서 `voice_design_prompt`, `head_padding`, `tail_padding`, `controlnet_pose`, `clothing_tags` 누락 → 스토리보드 재로드 시 null 덮어쓰기. ③ Spread Passthrough 패턴 규칙 수립 — CLAUDE.md에 Data Mapping Principles 추가, 8개 Whitelist 매핑 함수 전환 리스트업.
