...
| 8 | Character Builder 위저드 (Quick Start 템플릿 + 3-step: Basic→Appearance→LoRA, 카테고리 칩 그리드, LoRA 카드 브라우저, SD 프리뷰) | 기능 | [명세](FEATURES/CHARACTER_BUILDER.md) | [ ] |
...
### Phase G: Script 읽기 전용 리뷰 + 레이아웃 패치 (2026-02-13)

**문제**: (1) Script 탭과 Edit 탭 양쪽에서 씬 스크립트 편집 가능 → 중복 혼란. (2) Home 칸반 xl 브레이크포인트에서 QUICK STATS 우측 잘림.
**해결**: Script 탭 씬 목록을 읽기 전용 리뷰 뷰로 전환 (Agentic Human-in-the-loop 준비), 칸반 반응형 그리드 + overflow 방어.
...
## Phase 9: Agentic AI Pipeline (LangGraph Migration) — Discussion

**목표**: 대본 생성 파이프라인을 LangGraph 기반 에이전틱 AI로 전환. 반복 개선, 메모리, 자율 판단 도입.
**선행**: Phase 7-6 완료 (씬 편집 UX 안정화).
**명세**: [AGENTIC_PIPELINE.md](FEATURES/AGENTIC_PIPELINE.md)
**상태**: 논의 중 (Discussion) — 아키텍처 결정 필요

### 현행 문제

| 문제 | 현재 | 목표 |
|------|------|------|
| 단일 호출 | Gemini 1회 → 결과 | 생성 → 검토 → 수정 반복 루프 |
| 메모리 없음 | 세션 간 학습 불가 | 장기 메모리 (성공 패턴, 사용자 선호) |
| 복구 불가 | Phase 1 중단 시 처음부터 재시작 | Checkpoint 기반 어디서든 재개 |
| 이원화 | Script Gen / Creative Lab 코드 분리 | 단일 Graph 파이프라인으로 통합 |
| Human-in-the-loop | `step_review` 수동 구현 | LangGraph `interrupt()` 내장 |

### 단계별 계획

| Phase | 핵심 | 주요 작업 | 상태 |
|-------|------|----------|------|
| 0. Foundation | 인프라 | LangGraph + PostgresSaver/Store 설정, 최소 PoC | [ ] |
| 1. Script Graph | 핵심 전환 | ScriptState 설계, Research→Draft→Review→Revise→Finalize 그래프 구현 | [ ] |
| 2. Memory Layer | 학습 | 네임스페이스 설계, Learn 노드, 피드백 수집 UI | [ ] |
| 3. Creative 통합 | 레거시 전환 | Creative Lab 토론/파이프라인을 LangGraph 서브그래프로 전환 | [ ] |
| 4. 고도화 | 장기 | 병렬 실행, Streaming, 분산 큐, Self-improving | [ ] |

### 미결정 사항 (논의 필요)

| # | 질문 | 선택지 |
|---|------|--------|
| Q1 | LangChain 의존성 범위 | A: LangGraph만 (최소) / B: LangChain Core 포함 / C: 풀 LangChain |
| Q2 | Gemini 호출 방식 | A: 기존 `google-genai` 유지 / B: LangChain wrapper 전환 |
| Q3 | 기존 Creative 테이블 | A: 병행 / B: LangGraph 체크포인트로 마이그레이션 / C: 점진적 deprecated |
| Q4 | 단일 대본 생성도 Graph? | A: 항상 Graph / B: Quick 모드는 기존 단일 호출 유지 |
| Q5 | 반복 개선 비용 | Gemini 1회 → 3-4회 증가 허용 범위? |
...
- Phase 7-1: **26/27** 완료 (잔여: #8 Char Builder Wizard — [명세 최신화 완료](FEATURES/CHARACTER_BUILDER.md), Phase A-C 3단계)
...
- **Phase 7-6**: Scene UX Enhancement **완료** (Phase A~G: 씬 편집 탭/DnD/3-Column/Workspace 탭/네비 재설계/Publish 통합/레이아웃 통일/Script 읽기전용+칸반 패치)
- **Phase 9**: Agentic AI Pipeline — **논의 중** (아키텍처 결정 필요, [명세](FEATURES/AGENTIC_PIPELINE.md))
...
**Tier 1 — Character Builder 위저드 (7-1 #8)**

외부 사례 분석 반영 (NovelAI/Civitai/VRoid/Pixai/Character.ai). [상세 명세](FEATURES/CHARACTER_BUILDER.md)

| 순위 | Phase | 작업 | 난이도 |
|------|-------|------|--------|
| 4-A | A | 위저드 골격 + Quick Start 템플릿 + Step 1-2 (카테고리 칩 그리드, 인기순 강조) + Save | 반나절 |
| 4-B | B | Step 3 LoRA 카드 그리드 (썸네일+메타+인라인 weight) + Type/Gender 필터 | 2시간 |
| 4-C | C | `POST /characters/preview` + `assign-preview` API + SSE 프리뷰 생성 | 반나절 |

**Tier 1.5 — Feature 확장**
| 순위 | 출처 | 작업 | 근거 |
|------|------|------|------|
| 5 | 7-2 P2 | Channel DNA + Tag Intelligence | 프로젝트 차별화 |
| ~~6~~ | ~~7-1 #2~~ | ~~Setup Wizard~~ | ~~N/A (기존 기능으로 대체)~~ |

**Tier 2 — Agentic AI 전환 (논의 중)**
| 순위 | 출처 | 작업 | 근거 |
|------|------|------|------|
| 7 | 9 P0 | LangGraph Foundation + PoC | 대본 파이프라인 근본 개선 선행 |
| 8 | 9 P1 | Script Generation Graph 전환 | 반복 개선 + Checkpoint 도입 |
| 9 | 9 P2 | Memory Layer (장기 학습) | 에이전트 학습 능력 |
