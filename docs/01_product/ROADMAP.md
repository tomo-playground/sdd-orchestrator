...
- **Langfuse 연동 정상화** (2026-02-18): langfuse + langchain 패키지 설치로 CallbackHandler 생성 실패 해소. 전체 파이프라인 trace 기록 확인 (19 observations/trace, Gemini 토큰 사용량 추적). **오류 메시지 개선**: Review 노드 `user_summary` 필드 추가로 사용자용/AI용 메시지 분리 (errors는 Revise 정규식 호환 유지, user_summary는 Frontend 표시용), Gemini 안전 필터 에러를 원인별 구체적 메시지 + 해결 방법으로 분류, Frontend `ReasoningSections` 상세 보기 토글 추가. **Langfuse 트레이스 검수** (trace `9095a193`): Cinematographer/Research DB 세션 버그 발견 → 수정 완료, 서사 품질 개선점 3건 도출(Tier 1 등록)
- **Tool-Calling DB 세션 버그 수정** (2026-02-18): Cinematographer/Research 노드 `config["configurable"]["db"]` 미주입으로 Tool-Calling 전면 불가 → `get_db_session()` fallback 추가 (Writer/Revise와 동일 패턴). 테스트 업데이트 (83개 PASS)
- **Writer ↔ Critic 컨셉 연결 강화** (2026-02-18): Critic 선정 컨셉이 description 문자열에 혼합되어 Gemini가 무시하던 문제 해결. `selected_concept`을 `StoryboardRequest` 별도 필드로 분리, 템플릿 3개 + writer_planning에 `=== SELECTED CONCEPT (MANDATORY) ===` 최상단 필수 섹션 추가. `_partials/selected_concept.j2` partial 추출(DRY), `extract_selected_concept()` 공통 헬퍼(`state.py`), Revise 재생성 시 컨셉 보존. 테스트 16개 PASS (신규 4개: revise 보존, optional 필드, assertion 변경)
- **Agent Graceful Degradation 강화** (2026-02-18): Cinematographer/Critic 노드 error 설정 → `result: None` 반환으로 변경. Cinematographer: `validate_visuals()` 인자 버그(dict→list) + 키 버그(`"valid"`→`"ok"`) 수정, `_parse_scenes()` 순수함수 추출. Critic: error 3곳→`_EMPTY_RESULT` 반환 + feedback 덮어쓰기 수정(전 role 누적). Production fan-out(tts/sound/copyright) skip 방지. 테스트 1,792개 PASS
- **useScriptEditor 페이지 이탈 시 대본 소실 수정** (2026-02-19): `setField`/`updateScene` 호출 시 글로벌 스토어 `isDirty` 미반영 → 콘텐츠 필드 변경 시 즉시 `isDirty: true` 설정. unmount 시 `syncToGlobalStore` 호출로 페이지 이동 시 데이터 보존. `CONTENT_FIELDS` 상수 + `stateRef`/`dirtyRef` ref 패턴. `resume` stale closure 수정(`state.*` → `stateRef.current.*`), `loadStoryboard` 후 dirty 리셋. 18개 테스트 추가
- **테스트**: Backend 1,805 passed + Frontend 339 passed = **총 2,144개** (Phase 10: 94개, DB 세션 수정: 83개 PASS, Graceful Degradation: 14개 추가, useScriptEditor dirty tracking: 18개 추가)
...
**Tier 1 — Pipeline 서사 품질 (Langfuse 트레이스 검수 도출, 2026-02-18)**
| 순위 | 작업 | 근거 | 상태 |
|------|------|------|------|
| 8 | Writer ↔ Critic 컨셉 연결 강화 | Writer가 Critic 선정 컨셉을 무시하고 독자 생성 (trace `9095a193`) | [x] |
| 9 | 최소 씬 수 공식 재검토 (`ceil(duration/2)`) | 60초→30씬 강제, 3회 시도해도 미달→강제 통과 반복 | [x] |
| 10 | Revise 전략 개선 (씬 분할/확장) | 매번 전체 재생성(35~41초, $0.003)으로 비효율 + 품질 불안정 | [x] |
