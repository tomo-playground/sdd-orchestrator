...
| # | 작업 | 분류 | 상태 |
|---|------|------|------|
| 1 | Unified Setup Wizard: 프로젝트/그룹 생성 2-step 통합 위자드 (채널→시리즈→Studio), Home/Studio/ContextBar 트리거 5곳 연결 | UX | [x] |
...
| # | 작업 | 분류 | 상태 |
|---|------|------|------|
| 1 | 2-Column 대시보드 레이아웃 (단일 컬럼 → `grid-cols-[2fr_1fr]`, Quick Actions/Stats Above-the-fold 노출) | UX | [ ] |
| 2 | "Continue Working" 섹션 (최근 수정 스토리보드 2-3개, 워크플로우 단계 표시 Script/Edit/Publish, Continue CTA) | UX | [ ] |
| 3 | Video Gallery 컴팩트화 (그리드 → 수평 스크롤 캐러셀, 카드 높이 400px→220px, "모두 보기" 링크) | UX | [ ] |
...
| # | 작업 | 분류 | 상태 |
|---|------|------|------|
| 7 | Publish 3-Column 재배치: LEFT(Layout 선택+Render 버튼+Progress) / CENTER(비디오 프리뷰 크게+설정 접힘) / RIGHT(Caption/Likes+Recent Videos 리스트) | UX | [ ] |
| 8 | Render Settings `<details>` 래핑 (기본 접힘 상태, 설정보다 결과물 우선) | UX | [ ] |
...
- **Phase 7-Z**: Home Dashboard & Publish UX Redesign — **계획 완료**, Phase A 착수 예정. Home 2-Column 대시보드 + Continue Working + Gallery 컴팩트화 + Publish 비디오 센터 배치
- **Phase 9**: Agentic AI Pipeline — **Phase 0 Foundation 완료** (2026-02-13), **Phase 1 동등 전환 완료** (2026-02-15), **Phase 1.5 기능 확장 완료** (2026-02-15). 8-노드 조건 분기 그래프 (research→debate→draft→review→revise→human_gate→finalize→learn), Quick/Full 모드 + Preset 3종, Revise 루프 + Human Gate interrupt, reasoning [왜?] 패널, ReviewApprovalPanel. Phase 2 Memory + Obs 착수 예정. [명세](FEATURES/AGENTIC_PIPELINE.md)
- **VRT Baseline**: 24개 스크린샷, 8개 스펙 완료 (6-7 #2)
- **테스트**: Backend 1,576 passed (13 skipped) + Frontend 299 passed = **총 1,875개**
...
| 순위 | 출처 | 작업 | 기간 | 근거 |
|------|------|------|------|------|
| 5 | 7-Z A | Home 2-Column 레이아웃 + Continue Working + Gallery 컴팩트화 | 1-2일 | 비디오 갤러리 뷰어→창작 대시보드 전환, 기존 컴포넌트 재배치 |
| 6 | 7-Z C | Publish 탭 재배치 (비디오 프리뷰 센터, 설정 접힘) | 1일 | 결과물(비디오)이 구석에 밀려있는 문제 해결 |
