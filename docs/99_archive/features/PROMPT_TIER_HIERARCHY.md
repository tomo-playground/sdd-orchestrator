...
2. **DB 태그 + scene_positive_prompt 중복**: `_collect_character_tags()`에서 같은 태그 2번 추가
...
3. **`_collect_character_tags()` 중복 제거** — DB 태그 먼저 수집 → `seen_names` set 구성 → scene_positive_prompt에서 동일 태그 skip, 같은 group이면 custom이 DB 대체
