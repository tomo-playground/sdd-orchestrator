...
[Frontend Studio]                    [Backend]                         [SD WebUI]
 ...
 Zustand Store ────┤
   planSlice:      │
     ...
     basePromptA                     (character.scene_positive_prompt)
...
- `_collect_character_tags()`에서 DB 태그와 scene_positive_prompt 간 중복 제거 (동일 name → skip, 동일 group → custom 우선)
...
3. `character.scene_positive_prompt` → comma split → L2 (restricted 태그 필터링)
...
[Layer 1] StyleProfile.default_negative       ← 화풍/품질 제어 (SSOT)
    +
[Layer 2] character.scene_negative_prompt    ← 캐릭터 특화 억제
    +
...
| **캐릭터** | `characters.scene_negative_prompt` | 해당 캐릭터 특화 억제만 | `nsfw, old, lazy, suit` |
| **씬** | `scenes.negative_prompt` | 씬 고유 억제 | (일반적으로 비어 있음) |

**핵심 원칙**: `scene_negative_prompt`는 **캐릭터에만 해당하는 억제 태그**만 기재한다.
...
캐릭터 `scene_negative_prompt`에서 이 태그들을 **중복 기재하지 않는다**.
