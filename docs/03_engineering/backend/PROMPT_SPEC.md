# Prompt System Specification (v4.6)
...
| 버전 | 날짜 | 주요 변경사항 |
|------|------|--------------|
| v4.6 | 2026-03-02 | 12-Layer Tier Ownership: 3-Tier 소유권 규칙(LAYER_OWNERS), Quality L0 직접 주입(skip_quality), _collect_character_tags 중복 제거 |
| v4.5 | 2026-03-02 | Negative Prompt 3-레이어 원칙 명문화: StyleProfile=품질/화풍, Character=캐릭터 특화, normalize_negative_prompt 가중치 중복 제거, _ensure_framing_tag 규칙 추가 |
...
| v3.0 | 2026-01-30 | 12-Layer PromptBuilder, DB-driven 충돌 규칙 |
...
[Frontend Studio]                    [Backend]                         [SD WebUI]
 ...
 Zustand Store ────┤
   planSlice:      │
     ...
     basePromptA                     (character.positive_prompt)
...
`backend/services/prompt/composition.py` → `PromptBuilder`
...
### 3.1b 레이어 소유권 (Tier Ownership)

각 레이어는 **소유 Tier**가 정의되어 있으며, 해당 Tier만 태그를 주입할 수 있다.

| Tier | 소유자 | 레이어 | 상수 |
|------|--------|--------|------|
| **Tier 1** | StyleProfile | L0 (Quality), L11 (Atmosphere) | `config_prompt.LAYER_OWNERS` |
| **Tier 2** | Character | L1~L6 (Subject~Accessory) | |
| **Tier 3** | Scene | L7~L10 (Expression~Environment) | |

**규칙**:
- Quality 태그는 `compose_for_character(quality_tags=)` 파라미터로 L0에 직접 배치
- `apply_style_profile_to_prompt(skip_quality=True)`로 외부 quality prepend 차단 (이중 주입 방지)
- `_collect_character_tags()`에서 DB 태그와 `positive_prompt` 간 중복 제거 (동일 name → skip, 동일 group → custom 우선)
...
`scene_positive_prompt`에서 씬 프롬프트로 유입 차단하는 태그:
...
- DB `reference_positive_prompt` / `reference_negative_prompt` → 캐릭터 고유 태그만 저장 (공통 중복 금지).
- 새 캐릭터 생성 시 상수를 DB에 복사하지 않음 (NULL 유지).
- CLAUDE.md "캐릭터 프롬프트 SSOT" 원칙 참조.

### ~~Issue 9: 씬·레퍼런스 프롬프트 필드 분리로 인한 외형 불일치~~ (해결 완료 — Phase 30-K, 2026-03-07)

**근본 원인**: `scene_positive_prompt`, `scene_negative_prompt`, `reference_positive_prompt`, `reference_negative_prompt`, `common_negative_prompts` 5개 필드를 별도 관리 → 씬과 레퍼런스 외형 불일치.

**해결**: 5필드 → 2필드 통합.
- `positive_prompt`: 씬·레퍼런스 양쪽 `_collect_character_tags()`에서 동일하게 수집
- `negative_prompt`: 씬 경로(`generation_prompt.py`/`image_generation_core.py`) + 레퍼런스 경로(`reference.py`) 모두 동일 필드 사용
- `_parse_reference_tags()` dead code 제거 (레퍼런스 경로에서 `_collect_character_tags()` 통일)
- Alembic dedup 마이그레이션: 5필드 토큰 정규화 후 합산, 중복 제거
...
```
[Layer 1] StyleProfile.default_negative  ← 화풍/품질 제어 (SSOT)
    +
[Layer 2] character.negative_prompt      ← 캐릭터 특화 억제 (씬·레퍼런스 공용)
    +
[Layer 3] scene.negative_prompt          ← 씬별 추가 억제 (선택)
    ↓
normalize_negative_prompt()   ← 가중치 우선 중복 제거
    ↓
최종 negative prompt → SD WebUI
```
...
| 레이어 | 소유자 | 포함 내용 | 예시 |
|--------|--------|-----------|------|
| **StyleProfile** | `style_profiles.default_negative` | 화풍/품질 품질 보호 | `worst quality, bad anatomy, blurry, (cartoon:1.3), anime` |
| **캐릭터** | `characters.negative_prompt` | 해당 캐릭터 특화 억제만 (씬·레퍼런스 양쪽 공용) | `nsfw, old, lazy, suit` |
| **씬** | `scenes.negative_prompt` | 씬 고유 억제 | (일반적으로 비어 있음) |

**핵심 원칙**: `negative_prompt`는 **캐릭터에만 해당하는 억제 태그**만 기재한다.

- ✅ 허용: `nsfw`, 성격/행동 억제 (`lazy`, `overly_cheerful`), 외형 특화 억제 (`mature`, `wrinkles`), 캐릭터에 나오면 안 되는 복장/소품
- ❌ 금지: `bad anatomy`, `blurry`, `worst quality`, `EasyNegative`, `cartoon`, `anime` 등 품질/화풍 태그 → **StyleProfile이 담당**

### 11.3 중복 제거 규칙 (`normalize_negative_prompt`)

가중치 버전과 일반 버전이 동시에 존재하면 **가중치 버전 우선**:

```
입력: "cartoon, (cartoon:1.3), anime, bad anatomy"
출력: "(cartoon:1.3), anime, bad anatomy"   ← 중복 cartoon 제거, 가중치 버전 유지
```

동일 베이스 키(`_negative_base_key`)로 정규화 후 비교:
- `(cartoon:1.3)` → key: `cartoon`
- `cartoon` → key: `cartoon`  → 중복 → 가중치 버전 유지

### 11.4 실사(Realistic) 스타일 특화 규칙

Realistic StyleProfile에서는 아래 태그가 **StyleProfile.default_negative에만** 포함:

```
(anime:1.3), (cartoon:1.3), illustration, drawing, watercolor, sketch, super_deformed
```

캐릭터 `negative_prompt`에서 이 태그들을 **중복 기재하지 않는다**.

### 11.5 자동 보완 (`_ensure_framing_tag`)

`standing` 포즈 + framing 태그 없을 때 → `full_body` 자동 삽입 (다리 잘림 방지):

```python
# LAYER_ACTION에 "standing" 있고 LAYER_CAMERA에 framing 태그 없으면
layers[LAYER_CAMERA].insert(0, "full_body")
```

적용 지점: `compose_for_character()`, `compose_for_background()`, `compose_for_narrator()`, `compose_simple()`

### 11.6 코드 위치

| 기능 | 파일 | 함수 |
|------|------|------|
| 중복 제거 | `services/prompt/prompt.py` | `normalize_negative_prompt()` |
| 가중치 키 추출 | `services/prompt/prompt.py` | `_negative_base_key()` |
| Framing 보완 | `services/prompt/composition.py` | `_ensure_framing_tag()` |
| StyleProfile 파라미터 적용 | `services/generation.py` | `_adjust_parameters()` |
| 배경 생성 (Stage) | `services/stage/background_generator.py` | `_generate_background_image()` |
...
| Service | `services/prompt/service.py` | `generate_prompt_for_scene()` |
| Builder | `services/prompt/composition.py` | `compose_for_character()` |
| Tag info | `services/prompt/composition.py` | `get_tag_info()` |
| Flatten | `services/prompt/composition.py` | `_flatten_layers()` |
...
| 파일 | 커버리지 |
|------|----------|
| `tests/test_composition.py` | flatten/dedup, gender enhancement |
