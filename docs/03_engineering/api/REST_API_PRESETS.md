...
> v3.3: MusicGen 기반 AI BGM 프리셋 관리. 프롬프트 -> 음악 생성/미리듣기/저장.
...
MusicGen 모델을 사용하여 미리듣기 음악을 생성합니다. 결과는 임시 MediaAsset으로 등록됩니다.
...
MusicGen 모델을 수동으로 로딩합니다. 첫 preview 전에 호출하면 대기 시간을 줄일 수 있습니다.

**Response:**
```json
{"status": "ok", "message": "MusicGen model loaded"}
```
