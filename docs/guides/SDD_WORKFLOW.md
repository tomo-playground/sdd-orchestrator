...
[Claude] 테스트를 GREEN으로 만드는 코드 작성 → Stop Hook 자동 검증
  ...
  ↓
[병렬 리뷰]
  ├─ Claude 리뷰 (sdd-review.yml) — 설계 품질 검증
...
| 커밋 + push + PR | 풀 자율 | 로컬 |
| PR 리뷰 | 자동 | `sdd-review.yml` |
| 리뷰 피드백 수정 | 자동 | `sdd-fix.yml` auto-fix |
| @claude 수동 대응 | 자동 | `sdd-fix.yml` manual |
...
| 워크플로우 | 트리거 | 역할 | Runner |
|----------|--------|------|--------|
| `sdd-review.yml` | PR 생성/push | Claude 리뷰 (읽기 전용) | self-hosted |
| `sdd-fix.yml` (SDD Fix) | 리뷰 코멘트 / `@claude` 멘션 | 코드 자동 수정 | self-hosted |
| `sdd-sync.yml` | PR 머지 | 태스크 정리 + 브랜치 삭제 | self-hosted |
| `sentry-patrol.yml` | 매일 09:00 KST / 수동 | Sentry 에러 → GitHub Issue 자동 생성 | self-hosted |

### sdd-review.yml — PR 병렬 리뷰
